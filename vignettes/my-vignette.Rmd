...
Transforms cumulative hazard estimates to survival parameters specified by differential equations. We demonstrate how the package can be utilized in survival analysis.
...
where $F = (F_1,F_2,\cdots)$ is Lipschitz, two times continuously differentiable, and satisfies a linear growth bound. Several examples of such parameters can be found in [^fn1] [^fn2]. The main function *pluginEstimate* in this package estimate such parameters nonparametrically using cumulative hazard estimates obtained by Aalens additive hazard model. Among other things, *pluginEstimate* has the following input
...
$$ S_t = 1 - \int_0^t S_{s} dA_s.$$

We generate a sample of exponentially distributed survival times with 10% independent censoring, and calculate cumulative hazard estimates

```{r, fig.show='hold',results = "hide",echo=T}
n1 <- 300
Samp1 <- rexp(n1,1)
indepCens1 <- sample(0:1,n1,prob=c(0.1,0.9),replace=T)
startTimes1 <- rep(0,n1)

aaMod1 <- aalen(Surv(startTimes1,Samp1,indepCens1 == 1)~1)
```
...
$$ \begin{pmatrix} S_t \\ R_t \end{pmatrix} = \begin{pmatrix}1 \\ 0 \end{pmatrix} + \int_0^t \begin{pmatrix} -S_{s} & 0 \\ 0 & S_s \end{pmatrix}d \begin{pmatrix}A_s \\ s \end{pmatrix},$$
...
$$ J_{F_1}(x_1,x_2) = \begin{pmatrix} -1 & 0 \\ 0 & 0 \end{pmatrix}, J_{F_2}(x_1,x_2) = \begin{pmatrix} 0 & 0 \\ 1 & 0 \end{pmatrix}. $$
...
n2 <- 200
Samp2 <- rexp(n2,1.3)
indepCens2 <- sample(0:1,n2,prob=c(0.1,0.9),replace=T)
startTimes2 <- rep(0,n2)

aaMod2 <- aalen(Surv(startTimes2,Samp2,indepCens2 == 1)~1)
...
$$ RS_t = 1 + \int_0^t \begin{pmatrix} -RS_s & RS_s\end{pmatrix} d\begin{pmatrix} A_s^1 \\ A_s^2 \end{pmatrix}, $$
...
$$ \begin{pmatrix} S_t \\ C_t^1 \\ C_t^2 \end{pmatrix} = \begin{pmatrix} 1 \\ 0 \\0 \end{pmatrix} + \int_0^t \begin{pmatrix} -S_s & -S_s \\ S_s & 0 \\ 0 & S_s \end{pmatrix} d \begin{pmatrix} A^1_s \\ A^2_s \end{pmatrix}, $$
where $S$ is the survival. The first columns of $F$ are

$$F_1(x_1,x_2,x_2) = \begin{pmatrix} -x_1 \\x_1 \\ 0 \end{pmatrix},  F_2(x_1,x_2,x_2) = \begin{pmatrix} -x_1 \\ 0 \\ x_1 \end{pmatrix} $$ 

The reader may verify that the Jacobian matrix of $F_1$ and $F_1$ are
$$ J_{F_1}(x_1,x_2,x_3) = \begin{pmatrix} -1 & 0 & 0 \\1 & 0 & 0 \\ 0 & 0 & 0 \end{pmatrix} , J_{F_2}(x_1,x_2,x_3) = \begin{pmatrix} -1 & 0 & 0 \\0 & 0 & 0 \\ 1 & 0 & 0 \end{pmatrix} $$
...
# Add censoring
dfr$to.state[c(indepCens1,indepCens2) == 0] <- 0
